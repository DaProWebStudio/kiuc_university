#!/usr/bin/env bash
# Ежедневный дамп PostgreSQL-БД проекта КИЯК.
#
# Что делает:
#   1. Читает креды БД из .env (DATABASE_NAME/USER/PASSWORD/HOST)
#   2. Делает pg_dump в /var/www/kiuc_university/backups/kiuc-YYYYMMDD-HHMMSS.dump
...
PROJECT_DIR="/var/www/kiuc_university"
...
DUMP_FILE="${BACKUP_DIR}/kiuc-${STAMP}.dump"
...
DELETED=$(find "${BACKUP_DIR}" -maxdepth 1 -name 'kiuc-*.dump' -type f -mtime "+${KEEP_DAYS}" -print -delete | wc -l)
